...
try:
    ...
    st.register_type_strategy(pydantic2.EmailStr, st.emails().filter(is_valid_email))  # type: ignore[arg-type]
...
st.register_type_strategy(
    pydantic2.PyObject,  # type: ignore[arg-type]
    st.sampled_from(
        [cast(pydantic2.PyObject, f'math.{name}') for name in sorted(vars(math)) if not name.startswith('_')]
    ),
)
...
_color_regexes = (
    '|'.join(
        (
            pydantic2.color.r_hex_short,
            pydantic2.color.r_hex_long,
            pydantic2.color.r_rgb,
            pydantic2.color.r_rgba,
            pydantic2.color.r_hsl,
            pydantic2.color.r_hsla,
        )
    )
    # Use more precise regex patterns to avoid value-out-of-range errors
    .replace(pydantic2.color._r_sl, r'(?:(\d\d?(?:\.\d+)?|100(?:\.0+)?)%)')
    .replace(pydantic2.color._r_alpha, r'(?:(0(?:\.\d+)?|1(?:\.0+)?|\.\d+|\d{1,2}%))')
    .replace(pydantic2.color._r_255, r'(?:((?:\d|\d\d|[01]\d\d|2[0-4]\d|25[0-4])(?:\.\d+)?|255(?:\.0+)?))')
)
st.register_type_strategy(
    pydantic2.color.Color,
    st.one_of(
        st.sampled_from(sorted(pydantic2.color.COLORS_BY_NAME)),
        st.tuples(
            st.integers(0, 255),
            st.integers(0, 255),
            st.integers(0, 255),
...
def add_luhn_digit(card_number: str) -> str:
    ...
    for digit in '0123456789':
        with contextlib.suppress(Exception):
            pydantic2.PaymentCardNumber.validate_luhn_check_digit(card_number + digit)
...
st.register_type_strategy(
    pydantic2.PaymentCardNumber,
    st.from_regex('|'.join(card_patterns), fullmatch=True).map(add_luhn_digit),  # type: ignore[arg-type]
)

# UUIDs
st.register_type_strategy(pydantic2.UUID1, st.uuids(version=1))
st.register_type_strategy(pydantic2.UUID3, st.uuids(version=3))
st.register_type_strategy(pydantic2.UUID4, st.uuids(version=4))
st.register_type_strategy(pydantic2.UUID5, st.uuids(version=5))

# Secrets
st.register_type_strategy(pydantic2.SecretBytes, st.binary().map(pydantic2.SecretBytes))
st.register_type_strategy(pydantic2.SecretStr, st.text().map(pydantic2.SecretStr))

# IP addresses, networks, and interfaces
st.register_type_strategy(pydantic2.IPvAnyAddress, st.ip_addresses())  # type: ignore[arg-type]
st.register_type_strategy(
    pydantic2.IPvAnyInterface,
    st.from_type(ipaddress.IPv4Interface) | st.from_type(ipaddress.IPv6Interface),  # type: ignore[arg-type]
)
st.register_type_strategy(
    pydantic2.IPvAnyNetwork,
...
st.register_type_strategy(pydantic2.StrictBool, st.booleans())
st.register_type_strategy(pydantic2.StrictStr, st.text())


# FutureDate, PastDate
st.register_type_strategy(pydantic2.FutureDate, st.dates(min_value=datetime.date.today() + datetime.timedelta(days=1)))
st.register_type_strategy(pydantic2.PastDate, st.dates(max_value=datetime.date.today() - datetime.timedelta(days=1)))
...
@overload
def _registered(typ: Type[pydantic2.types.T]) -> Type[pydantic2.types.T]:
    pass


@overload
def _registered(typ: pydantic2.types.ConstrainedNumberMeta) -> pydantic2.types.ConstrainedNumberMeta:
    pass


def _registered(
    typ: Union[Type[pydantic2.types.T], pydantic2.types.ConstrainedNumberMeta]
) -> Union[Type[pydantic2.types.T], pydantic2.types.ConstrainedNumberMeta]:
    ...
    pydantic2.types._DEFINED_TYPES.add(typ)
...
def resolves(
    typ: Union[type, pydantic2.types.ConstrainedNumberMeta]
...
@resolves(pydantic2.JsonWrapper)
def resolve_json(cls):  # type: ignore[no-untyped-def]
    ...
    return st.builds(
        cls.inner_type.json if lenient_issubclass(inner_type, pydantic2.BaseModel) else json.dumps,
        inner,
...
@resolves(pydantic2.ConstrainedBytes)
...
@resolves(pydantic2.ConstrainedDecimal)
...
@resolves(pydantic2.ConstrainedFloat)
...
@resolves(pydantic2.ConstrainedInt)
...
@resolves(pydantic2.ConstrainedDate)
...
@resolves(pydantic2.ConstrainedStr)
...
for typ in list(pydantic2.types._DEFINED_TYPES):
    _registered(typ)
pydantic2.types._registered = _registered
st.register_type_strategy(pydantic2.Json, resolve_json)
